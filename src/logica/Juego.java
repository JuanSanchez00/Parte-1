...
public class Juego {
	...
	public Juego() {
		...
		this.horasInicio = calendario.get(Calendar.HOUR_OF_DAY);
		minutosInicio = calendario.get(Calendar.MINUTE);
		segundosInicio = calendario.get(Calendar.SECOND);
		System.out.println("EL JUEGO SE CREO A LAS "+horasInicio+";"+minutosInicio+";"+segundosInicio);
		inicializarJuego();
	...
	public int horasActual() {
		Calendar calendario = Calendar.getInstance();
		int horas = calendario.get(Calendar.HOUR_OF_DAY);
		if (horas<horasInicio) {
			return 60 - (horasInicio - horas);
		}
		return horas - horasInicio;
	}
	
	public int minutosActual() {
		Calendar calendario = Calendar.getInstance();
		int minutos = calendario.get(Calendar.HOUR_OF_DAY);
		return minutos - minutosInicio;
	}
	
	public int segundosActual() {
		Calendar calendario = Calendar.getInstance();
		int segundos = calendario.get(Calendar.HOUR_OF_DAY);
		System.out.println("SEGUNDOS INICIO: "+segundosInicio+" SEGUNDOS AHORA: "+segundos);
		int horas = calendario.get(Calendar.HOUR_OF_DAY);
		if (segundos<segundosInicio) {
			return 60 - (segundosInicio - segundos);
		}
		return segundos - segundosInicio;
	}
}
